...
//Christmas background structure
typedef struct
{
...
//People functions
void Christmas_People_SetFrame(void *user, u8 frame)
{
	Back_Christmas *this = (Back_Christmas*)user;
...
void Christmas_People_Draw(Back_Christmas *this, fixed_t x, fixed_t y)
{
	//Draw character
...
//Person functions
void Christmas_Person_SetFrame(void *user, u8 frame)
{
	Back_Christmas *this = (Back_Christmas*)user;
...
void Christmas_Person_Draw(Back_Christmas *this, fixed_t x, fixed_t y)
{
	//Draw character
...
//Fire functions
void Christmas_Fire_SetFrame(void *user, u8 frame)
{
	Back_Christmas *this = (Back_Christmas*)user;
...
void Christmas_Fire_Draw(Back_Christmas *this, fixed_t x, fixed_t y)
{
	//Draw character
...
void Back_Christmas_DrawFG(StageBack *back)
{
	Back_Christmas *this = (Back_Christmas*)back;
	
	//Draw shade
	RECT shade_src = {227,120,  1,  1};
...
void Back_Christmas_DrawBG(StageBack *back)
{
	Back_Christmas *this = (Back_Christmas*)back;
	...
	Animatable_Animate(&this->people_animatable, (void*)this, Christmas_People_SetFrame);
	
	Christmas_People_Draw(this, FIXED_DEC(15 - 3,1) - fx, FIXED_DEC(104 + 3,1) - fy);
	...
	Animatable_Animate(&this->person_animatable, (void*)this, Christmas_Person_SetFrame);
	
	Christmas_Person_Draw(this, FIXED_DEC(526 - 3,1) - fx, FIXED_DEC(105 + 6,1) - fy);
	...
	Animatable_Animate(&this->fire_animatable, (void*)this, Christmas_Fire_SetFrame);
	
	if (stage.stage_id != StageId_Spookpostor)
		Christmas_Fire_Draw(this, FIXED_DEC(315,1) - fx, FIXED_DEC(104,1) - fy);
	...
	RECT zoom0_src = {129, 56,125,127};
	RECT_FIXED zoom0_dst = {
		FIXED_DEC(187 - screen.SCREEN_WIDEOADD2,1) - fx,
		FIXED_DEC(81,1) - fy,
		FIXED_DEC(182 + screen.SCREEN_WIDEOADD,1),
		FIXED_DEC(184,1)
	};
	
	RECT zoom1_src = {  0, 56,125,127};
	RECT_FIXED zoom1_dst = {
		FIXED_DEC(367 - screen.SCREEN_WIDEOADD2,1) - fx,
		FIXED_DEC(81,1) - fy,
		FIXED_DEC(182 + screen.SCREEN_WIDEOADD,1),
		FIXED_DEC(184,1)
	...
	Stage_DrawTex(&this->tex_back1, &zoom1_src, &zoom1_dst, stage.camera.bzoom);
	Stage_DrawTex(&this->tex_back0, &zoom0_src, &zoom0_dst, stage.camera.bzoom);
}

void Back_Christmas_Free(StageBack *back)
{
	Back_Christmas *this = (Back_Christmas*)back;
...
StageBack *Back_Christmas_New(void)
{
	//Allocate background structure
	Back_Christmas *this = (Back_Christmas*)Mem_Alloc(sizeof(Back_Christmas));
	if (this == NULL)
		return NULL;
	
	//Set background functions
	this->back.draw_fg = Back_Christmas_DrawFG;
	this->back.draw_md = NULL;
	this->back.draw_bg = Back_Christmas_DrawBG;
	this->back.free = Back_Christmas_Free;
	
	//Load background textures
	IO_Data arc_back = IO_Read("\\BG\\CHRISTMS.ARC;1");
	Gfx_LoadTex(&this->tex_back0, Archive_Find(arc_back, "back0.tim"), 0);
	Gfx_LoadTex(&this->tex_back1, Archive_Find(arc_back, "back1.tim"), 0);
	Mem_Free(arc_back);
	
	//Load people textures
	this->arc_people = IO_Read("\\BG\\PEOPLE2.ARC;1");
